...
export interface UploadFormData {
  file: File;
  name: string;
  document_type: "devis" | "facture" | "contrat" | "autre";
  folder_id?: number;
  project_id?: number;
  client_id?: number;
  notes?: string;
...
export function UploadDocumentModal({ isOpen, onClose, onSubmit }: UploadDocumentModalProps) {
  ...
  const mockProjects = [
    ...
    { id: 2, name: "Installation équipement" },
  ];
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!selectedFile || !formData.name || !formData.document_type) return;
    ...
    try {
      ...
      setFormData({ document_type: "autre" });
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <CardHeader>
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-[#0F172A]">Uploader un document</h2>
            <button
              onClick={onClose}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    document_type: e.target.value as UploadFormData["document_type"],
                  })
                }
                className="w-full rounded-lg border border-[#E5E7EB] px-3 py-2 text-sm focus:border-[#F97316] focus:outline-none focus:ring-2 focus:ring-[#F97316] focus:ring-offset-1"
              >
