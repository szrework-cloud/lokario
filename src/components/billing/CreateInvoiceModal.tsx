...
export function CreateInvoiceModal({ isOpen, onClose, onSubmit }: CreateInvoiceModalProps) {
  ...
  const mockQuotes = [
    { id: 1, number: "DEV-2025-023", client_id: 1, amount: 1250 },
    { id: 2, number: "DEV-2025-020", client_id: 3, amount: 3200 },
  ];
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    quote_id: e.target.value ? Number(e.target.value) : undefined,
                    amount: e.target.value
                      ? mockQuotes.find((q) => q.id === Number(e.target.value))?.amount || 0
                      : formData.amount,
                  })
                }
                className="w-full rounded-lg border border-[#E5E7EB] px-3 py-2 text-sm focus:border-[#F97316] focus:outline-none focus:ring-2 focus:ring-[#F97316] focus:ring-offset-1"
