...
const InboxAnimation = ({ isActive }: { isActive: boolean }) => (
  <div className="relative w-full h-full flex items-center justify-center p-6">
    ...
    <div 
      className="w-full max-w-sm rounded-2xl bg-card/60 border border-border/40 overflow-hidden shadow-2xl backdrop-blur-sm"
      style={{
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-border/30 flex items-center justify-between bg-muted/30">
        ...
        <div className="flex items-center gap-2">
          <div 
            className="w-5 h-5 rounded-full bg-primary flex items-center justify-center text-[10px] text-primary-foreground font-bold"
            style={{
              opacity: isActive ? 1 : 0,
      ...
      <div className="divide-y divide-border/20">
        {[
          ...
          <div
            key={i}
            className={`px-4 py-3 flex items-start gap-3 ${msg.unread ? 'bg-primary/5' : ''}`}
            style={{
              opacity: isActive ? 1 : 0,
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <span className={`text-sm truncate ${msg.unread ? 'font-semibold text-foreground' : 'text-muted-foreground'}`}>
                  {msg.from}
                </span>
...
const InvoiceAnimation = ({ isActive }: { isActive: boolean }) => (
  <div className="relative w-full h-full flex items-center justify-center">
    ...
    {[0, 1, 2].map((i) => (
      <div
        key={i}
        className="absolute w-48 h-64 rounded-2xl bg-card/80 border border-primary/20 shadow-2xl backdrop-blur-sm"
        style={{
          right: `${20 + i * 30}px`,
          top: `${20 + i * 20}px`,
      ...
      >
        <div className="p-5 space-y-3 h-full flex flex-col">
          ...
          <div className="flex items-center justify-between">
            <div className="w-16 h-4 bg-primary/40 rounded" />
          ...
          <div className="pt-4 border-t border-border/30 flex justify-between items-center">
            ...
            <div className="w-20 h-4 bg-primary/50 rounded" />
          </div>
        </div>
      </div>
    ))}
    {/* Checkmark overlay */}
    <div 
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full bg-primary/20 flex items-center justify-center z-10"
      style={{
        opacity: isActive ? 1 : 0,
    ...
    >
      <svg className="w-10 h-10 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
      ...
      {[
        ...
        <g key={i}>
          <circle
            ...
            cy={node.y}
            r={i === 3 ? 20 : 12}
            fill={i === 3 ? "hsl(var(--primary))" : "hsl(var(--primary) / 0.3)"}
            stroke="hsl(var(--primary))"
            strokeWidth="2"
            style={{
              opacity: isActive ? 1 : 0,
      ...
      >
        ...
        <div 
          className={`w-7 h-7 rounded-lg border-2 flex items-center justify-center ${
            task.done ? 'bg-primary border-primary' : 'border-primary/40'
          }`}
          style={{
            backgroundColor: task.done && isActive ? 'hsl(var(--primary))' : 'transparent',
        ...
        <div className={`w-2 h-2 rounded-full ${
          ...
          task.priority === 'medium' ? 'bg-primary' : 'bg-muted-foreground/50'
        }`} />
      </div>
    ))}
  </div>
...
const CalendarAnimation = ({ isActive }: { isActive: boolean }) => (
  <div className="relative w-full h-full flex items-center justify-center p-4">
    <div 
      className="w-full max-w-xs rounded-2xl bg-card/70 border border-primary/20 overflow-hidden shadow-2xl backdrop-blur-sm"
      style={{
        opacity: isActive ? 1 : 0,
    ...
    >
      ...
      <div className="px-4 py-3 bg-primary/20 flex items-center justify-between">
        <span className="text-sm font-medium text-foreground">Décembre 2024</span>
      ...
      <div className="grid grid-cols-7 gap-1 px-3 py-2 border-b border-border/20">
        {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d, i) => (
          <span key={i} className="text-xs text-muted-foreground text-center">{d}</span>
        ))}
      </div>
      {/* Calendar grid */}
          ...
          return (
            <div
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs ${
                ...
                isHighlighted ? 'bg-primary/30 text-foreground' : 
      ...
      {/* Upcoming event */}
      <div 
        className="mx-3 mb-3 p-3 rounded-xl bg-primary/10 border border-primary/20"
        style={{
          opacity: isActive ? 1 : 0,
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-foreground">RDV Client - 14:00</span>
        </div>
      </div>
    </div>
  </div>
          ...
          <div className="relative h-[500px] flex items-center justify-center">
            {modules.map((module, index) => {
              ...
              return (
                ...
                >
                  {/* Card */}
                  <div className="relative rounded-3xl bg-card/30 backdrop-blur-sm border border-border/30 overflow-hidden">
                    {/* Glow effect */}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[400px]">
                      ...
                      <div className="relative p-10 lg:p-14 flex flex-col justify-center">
                        {/* Icon container */}
                        <div 
                          className="mb-8 inline-flex items-center justify-center w-16 h-16 rounded-xl bg-primary/10 border border-primary/20"
                          style={{
                        ...
                        {/* Content */}
                        <h3 className="relative text-3xl lg:text-4xl font-semibold text-foreground mb-4">
                          {module.title}
                        </h3>
                      ...
                      {/* Right: Animation */}
                      <div className="relative hidden lg:block border-l border-border/20">
