...
async def receive_messenger_webhook(
    ...
    try:
        ...
        # Identifier ou créer le client par ID Facebook
        from_id = data.get("from", {}).get("id")
        client = None
        if from_id:
            # Chercher par metadata ou créer
            # TODO: Stocker l'ID Facebook dans les métadonnées du client
            client = db.query(Client).filter(
                Client.company_id == company.id
            ).first()  # Simplifié pour l'instant
            
            if not client:
                client = Client(
                    company_id=company.id,
                    name=data.get("from", {}).get("name", f"User {from_id}"),
                    type="Client"
                )
                db.add(client)
                db.flush()
        ...
        message = InboxMessage(
            conversation_id=conversation.id,
            from_name=data.get("from", {}).get("name", "Inconnu"),
            content=data.get("message", ""),
            source="messenger",
            is_from_client=True,
            read=False,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error processing Messenger webhook: {str(e)}"
        )


# ===== VERIFICATION (pour Facebook Messenger) =====
...
async def verify_messenger_webhook(
    ...
    """
    # TODO: Vérifier le token depuis les settings de l'entreprise
